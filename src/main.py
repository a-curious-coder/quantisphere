...
def plot_data(crypto, data) -> None:
    ...
    layout = go.Layout(
        title=go.layout.Title(text=f'{crypto} prices this month', font=dict(color = 'white', size = 64, family='Arial'), x=0.05, y = 0.95),
        xaxis_title='Date',
    ...
    fig = go.Figure(data=go.Scatter(x=data['Date'], y=data['Close'], mode='lines'), layout=layout)
